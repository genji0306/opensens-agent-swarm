...
export async function startServer(): Promise<StartedServer> {
  ...
  // Start DRVP bridge if Redis is configured
  const drvpRedisUrl = process.env.REDIS_URL;
  const drvpCompanyId = process.env.DRVP_COMPANY_ID;
  if (drvpRedisUrl && drvpCompanyId) {
    import("./services/drvp-bridge.js")
      .then(({ startDrvpBridge }) =>
        startDrvpBridge({ redisUrl: drvpRedisUrl, companyId: drvpCompanyId }),
      )
      .then((bridge) => {
        logger.info("DRVP bridge started");
        process.once("SIGTERM", () => bridge.stop());
        process.once("SIGINT", () => bridge.stop());
      })
      .catch((err) => {
        logger.warn({ err }, "DRVP bridge failed to start (non-fatal)");
      });

    // Start DRVP → Paperclip issue auto-linker
    import("./services/drvp-issue-linker.js")
      .then(({ startDrvpIssueLinker }) => {
        const linker = startDrvpIssueLinker(db as any, drvpCompanyId);
        process.once("SIGTERM", () => linker.stop());
        process.once("SIGINT", () => linker.stop());
      })
      .catch((err) => {
        logger.warn({ err }, "DRVP issue auto-linker failed to start (non-fatal)");
      });
  }
