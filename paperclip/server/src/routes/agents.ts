...
export function agentRoutes(db: Db) {
  ...
  // ── Boost toggle (CEO/board only) ─────────────────────────────────
  router.post("/agents/:id/boost", async (req, res) => {
    assertBoard(req);
    const id = req.params.id as string;
    const existing = await svc.getById(id);
    if (!existing) {
    ...
    const boostEnabled = enabled !== false; // default to true if not specified
    const currentMeta = (existing.metadata as Record<string, unknown>) ?? {};
    const updatedMeta = { ...currentMeta, boostEnabled };

    const agent = await svc.update(id, { metadata: updatedMeta }, {});
    if (!agent) {
      res.status(404).json({ error: "Agent not found" });
      return;
    }
    ...
    await logActivity(db, {
      ...
      actorType: "user",
      actorId: req.actor.userId ?? "board",
      action: boostEnabled ? "agent.boost.enabled" : "agent.boost.disabled",
      entityType: "agent",
      entityId: agent.id,
      details: { boostEnabled },
    });

    res.json({ ...agent, boostEnabled });
  });

  router.get("/agents/:id/boost", async (req, res) => {
    const id = req.params.id as string;
    const existing = await svc.getById(id);
    if (!existing) {
      res.status(404).json({ error: "Agent not found" });
    ...
    res.json({ boostEnabled: meta.boostEnabled === true });
  });
