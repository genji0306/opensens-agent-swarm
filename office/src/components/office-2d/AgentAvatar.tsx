...
export const AgentAvatar = memo(function AgentAvatar({ agent }: AgentAvatarProps) {
  ...
  const issueId = useDrvpStore((s) => s.agentIssueMap.get(agent.name) ?? null);
  ...
  return (
    ...
    >
      ...
      {/* Task / issue badge (from DRVP agentIssueMap) */}
      {issueId && (
        <g transform={`translate(${-r * 0.6}, ${r * 0.5})`}>
          <foreignObject x={-24} y={-8} width={48} height={16} style={{ pointerEvents: "auto" }}>
            <a
              href={paperclipUrl ? `${paperclipUrl}/issues/${issueId}` : undefined}
              target="_blank"
            ...
            >
              <span
                style={{
                  ...
                  backgroundColor: isDark ? "rgba(30,58,138,0.7)" : "rgba(219,234,254,0.9)",
                  borderRadius: "3px",
                  padding: "1px 4px",
                  whiteSpace: "nowrap",
                }}
