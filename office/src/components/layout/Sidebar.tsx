...
export function Sidebar() {
  ...
  const pendingApprovals = usePaperclipStore((s) => s.dashboard?.pendingApprovals ?? 0);
  ...
  const paperclipSection = getSection("paperclip");
  ...
  return (
    <aside className="flex w-80 flex-col border-l border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      {/* Paperclip governance — collapsed by default */}
      <CollapsibleSection
        id="paperclip"
        title={t("sidebar.paperclipTitle")}
        collapsed={paperclipSection.collapsed}
        onToggle={() => toggleSection("paperclip")}
        height={paperclipSection.height}
        onHeightChange={(h) => setSectionHeight("paperclip", h)}
        minHeight={100}
        maxHeight={400}
